...
During implementation, our group also encountered several challenges that affected the final design. One challenge was that syllabi are not always formatted in the same way. Some syllabi provide specific dates, while others only list weekly topics or general deadlines. Because of this, we had to design the system to handle both exact dates and weekly schedules. Another challenge was that the system is not connected to the official course selection platform, which means it cannot automatically update when professors change deadlines or when new syllabus versions are uploaded. As a result, the accuracy of Syllabuddy depends on the uploaded syllabi for the current semester.
